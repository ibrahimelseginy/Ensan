...
|--------------------------------------------------------------------------
| GET /api/v1/mobile
| Returns a list of all available mobile endpoints.
*/
Route::get('/', function () {
...
| Returns all home sections: Heroes, Gallery, Services (قسم خدماتنا),
| Share What You Don't Need (شارك بما لا تحتاجه),
| Seasonal Campaigns (حملات موسمية), Final Section, About Us (معلومات عنا)
*/
Route::get('/home', [MobileApiController::class, 'getHomeContent']);

/*
|--------------------------------------------------------------------------
| 1.5 About Us (معلومات عنا)
|--------------------------------------------------------------------------
...
/*
|--------------------------------------------------------------------------
| 2. Projects & Campaigns (المشاريع والحملات)
|--------------------------------------------------------------------------
| GET /api/v1/mobile/projects  → List all projects enabled for mobile
| GET /api/v1/mobile/campaigns → List all campaigns enabled for mobile
*/
...
/*
|--------------------------------------------------------------------------
| 3. App News (أخبار التطبيق)
|--------------------------------------------------------------------------
| GET  /api/v1/mobile/news → Retrieve all published news articles
| POST /api/v1/mobile/news → Add a new news article
|
| POST Request Body:
...
/*
|--------------------------------------------------------------------------
| 4. Volunteer Requests (طلبات التطوع)
|--------------------------------------------------------------------------
| GET  /api/v1/mobile/volunteer-requests → View all volunteer requests (inbox)
| POST /api/v1/mobile/volunteer          → Submit a new volunteer request
|
| POST Request Body:
...
/*
|--------------------------------------------------------------------------
| 5. Case Applications (تقديم لحالة مستحقة)
|--------------------------------------------------------------------------
| POST /api/v1/mobile/case-application
|
| Supports:
|   - مشروع زاد الأيتام  → case_type: "zad"
|   - مشروع بعثاء الأمل  → case_type: "hope"
|   - Other types         → case_type: "medical" | "financial" | etc.
|
| POST Request Body:
|   - applicant_name    (required|string)
|   - applicant_phone   (required|string)
|   - case_type         (required|string)  → "zad" | "hope" | "medical" | ...
|   - description       (required|string)
|   - governorate       (optional|string)
|   - city              (optional|string)
|   - address           (optional|string)
|   - id_image          (optional|file|image)
|   - medical_report    (optional|file)
*/
...
/*
|--------------------------------------------------------------------------
| 6. Guest House Booking - Dar Al-Diyafa (دار الضيافة)
|--------------------------------------------------------------------------
...
|   - expected_duration (required|string) → "less_than_week" | "one_week" | "two_weeks" | ...
|   - medical_center    (optional|string)
|   - notes             (optional|string)
|   - patient_id_file   (optional|file)
*/
...
/*
|--------------------------------------------------------------------------
| 7. Push Notifications (إشعارات التطبيق)
|--------------------------------------------------------------------------
| GET /api/v1/mobile/notifications
| Returns all sent push notifications ordered by most recent.
| Each notification includes title, body, image_url, and sent_at timestamp.
*/
Route::get('/notifications', [MobileApiController::class, 'getNotifications']);

/*
|--------------------------------------------------------------------------
| 8. Donation Submission (طلب تبرع - موبايل)
|--------------------------------------------------------------------------
...
|   - donor_address   (optional|string)
|   - donation_amount (required|numeric)
|   - donation_for    (required|string)  → "General", "Campaign #12", "Project #5"
|   - payment_method  (required|string)  → "Cash", "Card", "Fawry", "Wallet"
|   - notes           (optional|string)
*/
