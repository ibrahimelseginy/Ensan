...
<div class="row align-items-center mb-5 mt-2">
    <div class="col-md-6 text-white">
        <h2 class="fw-800 mb-1">خدمات كفر الشيخ</h2>
        <p class="text-slate-400 mb-0">إدارة قاعدة بيانات مقدمي الخدمات الفنية والمهنية</p>
    </div>
    <div class="col-md-6 text-end">
        <button type="button" class="btn btn-primary rounded-pill px-4 py-2 fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#createModal">
            <i class="bi bi-plus-lg me-1"></i> إضافة خدمة جديدة
        </button>
    </div>
</div>
            ...
            <div class="service-card p-4">
                <div class="d-flex align-items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <span class="badge bg-secondary rounded-pill px-3 py-1 small opacity-75">{{ $item->service_type ?? 'مقدم خدمة' }}</span>
                    </div>
                </div>
                ...
                <div class="d-flex gap-2 pt-2 border-top border-white border-opacity-10">
                    ...
                    <form method="POST" action="{{ route('kafr-el-sheikh-services.destroy', $item) }}" 
            ...
            <form method="POST" action="{{ route('kafr-el-sheikh-services.store') }}">
                ...
                <div class="modal-body" style="background-color: #0f172a !important; opacity: 1 !important;">
                    <div class="mb-4 text-end">
                        ...
                        <input type="text" name="name" class="form-control" required placeholder="أدخل اسم الشخص أو المحل">
                    </div>
                    <div class="mb-4 text-end">
                        <label class="form-label">نوع الخدمة</label>
                        <input type="text" name="service_type" class="form-control" placeholder="مثلاً: سباكة، كهرباء، صيانة">
                    </div>
                    <div class="mb-4 text-end">
...
<div class="modal fade dark-modal-custom" id="editModal{{ $item->id }}" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="background-color: #0f172a !important; opacity: 1 !important;">
            <form method="POST" action="{{ route('kafr-el-sheikh-services.update', $item) }}">
                @csrf
                @method('PUT')
                <div class="modal-header bg-primary text-white border-0" style="background-color: #0066ff !important;">
                ...
                <div class="modal-body" style="background-color: #0f172a !important; opacity: 1 !important;">
                    ...
                    <div class="mb-4 text-end">
                        <label class="form-label">نوع الخدمة</label>
                        <input type="text" name="service_type" class="form-control" value="{{ $item->service_type }}">
                    </div>
                    <div class="mb-4 text-end">
                        <label class="form-label">رقم التليفون</label>
