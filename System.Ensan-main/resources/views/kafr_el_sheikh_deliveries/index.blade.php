...
<style>
    ...
    .delivery-card:hover .card-icon-wrapper {
    ...
    body:not(.theme-dark) .delivery-card {
        background: white;
        border-color: #e2e8f0;
        box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);
    }

    body:not(.theme-dark) .driver-name {
        color: #0f172a;
    }

    body:not(.theme-dark) .vehicle-badge {
        background: #f1f5f9;
        color: #475569;
        border-color: #e2e8f0;
    }
...
<div class="page-header-premium">
    <div class="header-content row align-items-center">
        <div class="col-md-7">
            <h1 class="title-gradient mb-2">توصيلات كفر الشيخ</h1>
            <p class="text-slate-400 mb-0 fs-5">إدارة قاعدة بيانات سيارات التوصيل والسائقين بكفاءة عالية</p>
        </div>
            ...
            <div class="delivery-card">
                <div class="d-flex justify-content-between align-items-start mb-3">
                    ...
                    <span class="vehicle-badge">
                        ...
                        {{ $item->vehicle_type ?? 'سائق' }}
                    </span>
                </div>

                <h3 class="driver-name">{{ $item->name }}</h3>
                ...
                <div class="card-actions">
                    ...
                    <form method="POST" action="{{ route('kafr-el-sheikh-deliveries.destroy', $item) }}" class="d-inline flex-grow-1" onsubmit="return confirm('هل أنت متأكد من حذف هذا السائق نهائياً؟')">
                        @csrf
                        @method('DELETE')
    ...
    @empty
        <div class="col-12">
            <div class="empty-state-container text-center py-5">
                <img src="{{ asset('images/empty-state.svg') }}" alt="No Deliveries" style="width: 200px; opacity: 0.5; margin-bottom: 20px;">
                <h3 class="text-white fw-bold mb-3">لا توجد سجلات توصيل حالياً</h3>
                <p class="text-slate-400 mb-4 fs-5">لم تقم بإضافة أي سائقين أو سيارات توصيل لكفر الشيخ حتى الآن.</p>
                <button type="button" class="btn-premium-add" data-bs-toggle="modal" data-bs-target="#createModal">
                    <i class="bi bi-plus-circle-fill fs-5"></i>
...
<div class="modal fade premium-modal" id="createModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST" action="{{ route('kafr-el-sheikh-deliveries.store') }}">
                @csrf
                ...
                <div class="modal-body">
                    <div class="form-floating-custom text-end">
                        <label>اسم السائق <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control" required placeholder="مثال: محمد أحمد">
                    </div>
                    <div class="form-floating-custom text-end">
                        <label>نوع المواصلة / السيارة</label>
                        <input type="text" name="vehicle_type" class="form-control" placeholder="مثال: سيارة ربع نقل, دراجة نارية">
                    </div>
                    <div class="form-floating-custom text-end">
                ...
                <div class="modal-footer flex-row-reverse">
                    <button type="submit" class="btn-save">
                        <i class="bi bi-check2-circle me-1"></i> حفظ السائق
                    </button>
...
<div class="modal fade premium-modal" id="editModal{{ $item->id }}" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST" action="{{ route('kafr-el-sheikh-deliveries.update', $item) }}">
                @csrf
                @method('PUT')
                <div class="modal-header">
                    <h5 class="modal-title">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-floating-custom text-end">
                        <label>نوع المواصلة / السيارة</label>
                        <input type="text" name="vehicle_type" class="form-control" value="{{ $item->vehicle_type }}">
                    </div>
                    <div class="form-floating-custom text-end">
                        <label>رقم التليفون للتواصل</label>
